...
export function buildTestSpace(scene: THREE.Scene): THREE.Group {
  ...
  const wallA = box('wallA', 5, 3, 0.4, wallMaterial, -3, 1.5, -8);
  const wallB = box('wallB', 4, 5, 0.4, wallMaterial, 2.5, 2.5, -11);
  const wallC = box('wallC', 6, 2, 0.4, wallMaterial, 8, 1, -6);
  const wallTall = box('wallTall', 4, 6, 0.4, wallMaterial, -8, 3, -4);
  group.add(wallA, wallB, wallC, wallTall);

  // Ledges: protruding boxes at varied heights on the walls.
  const ledge1 = box('ledge1', 1.6, 0.25, 0.7, ledgeMaterial, -4, 1.2, -7.7);
  const ledge2 = box('ledge2', 1.6, 0.25, 0.7, ledgeMaterial, -2, 2.2, -7.7);
  const ledge3 = box('ledge3', 1.4, 0.25, 0.6, ledgeMaterial, 2.5, 3.2, -10.6);
  const ledge4 = box('ledge4', 1.4, 0.25, 0.6, ledgeMaterial, -7.6, 2.0, -2.5);
  ...
  const overhang1 = box('overhang1', 2.4, 0.3, 1.8, wallMaterial, 0.5, 2.6, -6);
  overhang1.rotation.x = Math.PI / 2 - 0.5; // ~29° past horizontal, lip toward spawn
  const overhang2 = box('overhang2', 2.0, 0.3, 1.6, wallMaterial, -6, 4.2, -6);
  overhang2.rotation.x = Math.PI / 2 - 0.35;
  group.add(overhang1, overhang2);

  // Angled face: a steep slab to test diagonal footing.
  const angleFace = box('angleFace', 3, 0.25, 2.2, wallMaterial, 6, 2.2, -10);
  ...
  const gapLeft = box('gapLeft', 2.2, 0.9, 2.2, gapEdgeMaterial, -1.85, 0.45, -3.5);
  const gapRight = box('gapRight', 2.2, 1.1, 2.2, gapEdgeMaterial, 1.85, 0.55, -3.5);
